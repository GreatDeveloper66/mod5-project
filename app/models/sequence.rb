  ...
  has_many :asanas, through: :asanasequences
   ...
   def findhighestorder
    ...
    def remove_asana(asana_id)
		asanaseqs = self.asanasequences.find_all { |asanaseq| asanaseq.id == asana_id }
		asanaseqs.destroy_all
	end
	
	
	def remove_asana_order(order)
		asanaseqs = self.asanasequences.find { |asanaseq| asanaseq.order == order }
		asanaseqs.destroy_all
	end
	
	
	def change_asana_order(order:0, neworder:0)
		asanaseqs = self.asanasequences.find { |asanaseq| asanaseq.order == neworder }
		asanaseqs.update({order: order})
	end
	
	def flip_asanas_order(order1: 0, order2: 0)
		asanaseq1 = self.asanasequences.find { |asanaseq| asanaseq.order == order1 }
		asanaseq2 = self.asanasequences.find { |asanaseq| asanaseq.order == order2 }
